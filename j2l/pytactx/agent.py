...
class Agent(IAgent):
	def __init__(self,playerId:str or None=None, arena:str or None=None, username:str or None=None, password:str or None=None, server:str or None=None, port:int=1883, imgOutputPath:str or None="img.jpeg", autoconnect:bool=True, useProxy:bool=True, verbosity:int=3, robotId:str or None="_", welcomePrint:bool=True):
		...
		if ( welcomePrint ):
			...
			print("Hi there 👋")
			print("To see the arena in your web browser 🎮, go to 👉 https://play.jusdeliens.com/?viewer=pytactx&arena="+str(arena)+"&url="+str(server)+"&usr="+str(username)+"&pwd="+str(passwordParam))
	...
	def moveTowards(self,x:int,y:int) -> None:
		if ( type(x) is not int or type(y) is not int ):
			anx.warning("⚠️ move x,y values must be integer !")
			return
		if ( x < 0 or y < 0 ):
			anx.warning("⚠️ move x,y values must be postive !")
			return
	...
	def setColor(self, r:int, g:int, b:int) -> None:
		...
		if ( r<0 or r>255 or g<0 or g>255 or b<0 or b>255 ):
			anx.warning("⚠️ move r,b,g values must be included in [0;255] !")
			return
	...
	def addEventListener(self,attributeName:str, callback:Callable[[Any,str,Any,Any], None]) -> None:
		if ( attributeName not in self.__onAttributeChangeCallbacks ):
			...
			return
		self.__onAttributeChangeCallbacks[attributeName] = callback
	...
	def _onUpdated(self, eventSrc:Any, eventName:str, eventValue:Any):
		if ( self.__firstArenaRx != None and self.__firstArenaRx == True ):
			...
			self.robot.requestPlayer("nExe", fetchSources())
...
class AgentFr(IAgentFr):
	def __init__(self,nom:str or None=None, arene:str or None=None, username:str or None=None, password:str or None=None, url:str or None=None, port:int=1883, fluxImage:str or None="img.jpeg", autoconnect:bool=True, proxy:bool=True, verbosite:int=3, robotId:str or None="_", welcomePrint=True):
		...
		if ( welcomePrint ):
			passwordParam = (base64.b64encode(password.encode('utf-8'))).decode('utf-8')
			print("Hey 👋")
			print("Pour visualiser l'arene de jeu 🎮, allez sur 👉 https://play.jusdeliens.com/?viewer=pytactx&arena="+str(arene)+"&url="+str(url)+"&usr="+str(username)+"&pwd="+str(passwordParam))
		...
		self.__agent = Agent(nom, arene,username,password,url,port,fluxImage, autoconnect,proxy,verbosite, robotId, False)
